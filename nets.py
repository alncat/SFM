...
def disp_bottleneck_dense_sep(end_points, concat, scope, skip_block, in_channels, out_channels, reuse=False, rates=[2,4], use_skip=True, use_skip2=True, skip2=None, activation_fn=tf.nn.elu):
    with tf.variable_scope(scope, reuse=reuse):
        iH, iW = concat.get_shape()[1], concat.get_shape()[2]
        upnet = tf.image.resize_bilinear(concat, [iH*2, iW*2])
        upnet = slim.conv2d(upnet, in_channels, [1, 1], activation_fn=None)
        supnet = slim.conv2d(upnet, out_channels//2, [3, 3])
        aspp_up = slim.conv2d(upnet, out_channels//2, [3, 3], rate=rates[0])
        aspp_up2 = slim.conv2d(upnet, out_channels//2, [3, 3], rate=rates[1])
        if use_skip:
            skip = slim.batch_norm(end_points[skip_block], activation_fn=activation_fn)
            skip = slim.conv2d(skip, out_channels//2, [1, 1])
        else:
            skip = slim.conv2d(tf.concat([aspp_up, aspp_up2], axis=3), out_channels//2, [3, 3], rate=rates[1]*2)
        if use_skip2:
            skip2 = tf.image.resize_bilinear(skip2, [iH*2, iW*2])
            skip2 = slim.conv2d(skip2, out_channels//2, [1, 1])
            xconcat = tf.concat([supnet, aspp_up, aspp_up2, skip, skip2], axis=3)
        else:
            xconcat = tf.concat([supnet, aspp_up, aspp_up2, skip], axis=3)
        xconcat = slim.conv2d(xconcat, out_channels, [1, 1], activation_fn=None)
...
def disp_aspp_u_dense_sep(inputs, args, is_training, reuse, size, custom_getter=None):
    out_depth = 256
    with slim.arg_scope(resnet_utils.resnet_arg_scope(args.l2_regularizer, is_training,
                                                      args.batch_norm_decay,
                                                      args.batch_norm_epsilon,
        ...
        with tf.variable_scope("DeepLab_v3", reuse=reuse, custom_getter=custom_getter):
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose, slim.separable_conv2d]):
                ...
                upnet = slim.conv2d(upnet, 1024, [1, 1], activation_fn=None)
                #upnet1 = slim.batch_norm(net, activation_fn=tf.nn.elu)
                concat, _ = disp_bottleneck_dense_sep(end_points, upnet, "concat", args.resnet_model+"/block3", 1024, 512, reuse=reuse, use_skip2=False)
                concat1, skip1 = disp_bottleneck_dense_sep(end_points, concat, "concat1", args.resnet_model+"/block2", 512, 256, reuse=reuse, use_skip2=False)
                disp1 = DISP_SCALING*slim.conv2d(concat1, 1, [3, 3], activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp1') + MIN_DISP

                xconcat2, skip2 = disp_bottleneck_dense_sep(end_points, concat1, "concat2", args.resnet_model+"/block1", 256, 128, reuse=reuse, skip2=skip1)
                disp2 = DISP_SCALING*slim.conv2d(xconcat2, 1, [3, 3], activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp2') + MIN_DISP

                #upsampe3
                xconcat3, skip3 = disp_bottleneck_dense_sep(end_points, xconcat2, "concat3", args.resnet_model+"/root_block", 128, 64, reuse=reuse, rates=[3,6], use_skip=False, skip2=skip2)
                #for output
                disp3 = DISP_SCALING*slim.conv2d(xconcat3, 1, [3, 3], stride=1, activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp3') + MIN_DISP

                #upsample4
                xconcat4, _ = disp_bottleneck_dense_sep(end_points, xconcat3, "concat4", args.resnet_model+"/root_block", 64, 32, reuse=reuse, rates=[3,6], use_skip=False, skip2=skip3)

                #for output
                disp4  = DISP_SCALING * slim.conv2d(xconcat4, 1,   [3, 3], stride=1,
                    activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp4') + MIN_DISP

                return [disp4, disp3, disp2, disp1], end_points
import xception
def disp_aspp_u_dense_x(inputs, args, is_training, reuse, size, custom_getter=None):
    out_depth = 256
    with slim.arg_scope(xception.xception_arg_scope(args.l2_regularize,
                                                    is_training,
                                                    args.batch_norm_decay,
                                                    args.batch_norm_epsilon
                                                      )):
        args.resnet_model = "xception_41"
        _, end_points = xception.xception_41(inputs,
                               is_training=is_training,
                               global_pool=False,
                               spatial_squeeze=False,
                               output_stride=32,
        ...
        with tf.variable_scope("DeepLab_v3", reuse=reuse, custom_getter=custom_getter):
            ...
            # resize the preact features
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose, slim.separable_conv2d]):
                upnet = slim.batch_norm(net, activation_fn=tf.nn.relu)
                upnet = slim.conv2d(upnet, 1024, [1, 1], activation_fn=None)
                #upnet1 = slim.batch_norm(net, activation_fn=tf.nn.elu)
                concat, _ = disp_bottleneck_dense_sep(end_points, upnet, "concat", args.resnet_model+"/middle_flow/block1", 1024, 512, reuse=reuse, use_skip2=False,activation_fn=tf.nn.relu)
                concat1, skip1 = disp_bottleneck_dense_sep(end_points, concat, "concat1", args.resnet_model+"/entry_flow/block2", 512, 256, reuse=reuse, use_skip2=False,activation_fn=tf.nn.relu)
                xconcat2, skip2 = disp_bottleneck_dense_sep(end_points, concat1, "concat2", args.resnet_model+"/entry_flow/block1", 256, 128, reuse=reuse, skip2=skip1,activation_fn=tf.nn.relu)
                disp2 = DISP_SCALING*slim.conv2d(xconcat2, 1, [3, 3], activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp2') + MIN_DISP

                #upsampe3
                xconcat3, skip3 = disp_bottleneck_dense_sep(end_points, xconcat2, "concat3", args.resnet_model+"/root_block", 128, 64, reuse=reuse, rates=[3,6], use_skip=False, skip2=skip2,activation_fn=tf.nn.relu)
                #for output
                disp3 = DISP_SCALING*slim.conv2d(xconcat3, 1, [3, 3], stride=1, activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp3') + MIN_DISP

                #upsample4
                xconcat4, _ = disp_bottleneck_dense_sep(end_points, xconcat3, "concat4", args.resnet_model+"/root_block", 64, 32, reuse=reuse, rates=[3,6], use_skip=False, skip2=skip3,activation_fn=tf.nn.relu)
