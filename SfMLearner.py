...
class SfMLearner(object):
    ...
    def build_train_graph(self, tgt_image, src_image_stack, intrinsics, is_training = True, reuse=False):
        ...
        if not self.use_cspn:
            ...
            pred_disp, depth_net_endpoints = disp_net(tgt_image, is_training, reuse)
        ...
        for s in range(opt.num_scales):
            ...
            for i in range(opt.num_source):
                ...
                curr_proj_image, _ = projective_inverse_warp(
                    curr_src_image_stack[:,:,:,3*i:3*(i+1)],
                    #curr_tgt_image,
                    tf.squeeze(pred_depth[s], axis=3),
                    pred_poses[:,i,:],
                    intrinsics[:,s,:,:])
                curr_proj_src_image, _ = projective_warp(
                    curr_tgt_image,
                    tf.squeeze(pred_depth[s], axis=3),
                    pred_poses[:,i,:],
                    intrinsics[:,s,:,:])
                ...
                if opt.explain_reg_weight > 0:
                    ...
                    pixel_loss += tf.reduce_mean(curr_proj_error) #+ (1-alpha)*tf.reduce_mean(curr_ssim_error)
    ...
    def build_depth_test_graph(self):
        ...
        with tf.name_scope("depth_prediction"):
            if self.use_cspn:
                ...
                pred_disp, depth_net_endpoints = disp_net(input_mc, False, False)
