...
def bilinear_deep(net, scope, in_depth=256, rates=[2,3,5], depth=256, reuse=None, activation_fn=tf.nn.elu):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        at_pool3x3_1 = utils.bilinear_conv2d(net, "conv_3x3_1", 3, in_depth, depth, rate=rates[0], reuse=reuse, activation_fn=activation_fn)

        at_pool3x3_2 = utils.bilinear_conv2d(at_pool3x3_1, "conv_3x3_2", 3, depth, depth, rate=rates[1], reuse=reuse, activation_fn=activation_fn)

        at_pool3x3_3 = utils.bilinear_conv2d(at_pool3x3_2, "conv_3x3_3", 3, depth, depth, rate=rates[2], reuse=reuse, activation_fn=activation_fn)
...
def disp_dense_aspp(inputs, args, is_training, reuse, size):
    out_depth = 256
    with slim.arg_scope(resnet_utils.resnet_arg_scope(args.l2_regularizer, is_training,
                                                      args.batch_norm_decay,
                                                      args.batch_norm_epsilon,
