...
class SfMLearner(object):
    ...
    def build_train_graph(self, tgt_image, src_image_stack, intrinsics, is_training = True, reuse=False):
        ...
        for s in range(opt.num_scales):
            ...
            for i in range(opt.num_source):
                ...
                curr_error = alpha*tf.abs(curr_proj_image - curr_tgt_image)
                ...
                #curr_ssim_error = proj_mask*tf_ssim(curr_proj_image, curr_tgt_image, size=3, mean_metric=False)
                curr_error += (1 - alpha)*tf_ssim(curr_proj_image, curr_tgt_image, size=3, mean_metric=False)
                curr_errors.append(curr_error)
                curr_ssim_error = proj_src_mask*tf_ssim(curr_proj_src_image, curr_src_image_stack[:,:,:,3*i:3*(i+1)], size=3, mean_metric=True)
                # Cross-entropy loss as regularization for the
                # explainability prediction

                # Photo-consistency loss weighted by explainability
                pixel_loss += alpha*tf.reduce_mean(curr_proj_error) + (1-alpha)*tf.reduce_mean(curr_ssim_error)
                #pixel_loss += tf.reduce_mean(curr_ssim_error)
            ...
            pixel_loss += 2.*tf.reduce_mean(tf.minimum(curr_errors[0], curr_errors[1]))
    ...
    def train(self, opt):
        ...
        variables_resnet = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=args.resnet_model)
        exclude_variables = [args.resnet_model + "/logits"]
        variables_to_restore = []
        for var in variables_resnet:
            if 'Adam' in var.name:
                continue
            if var.name not in exclude_variables:
