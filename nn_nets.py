        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            if do_trans:
                with tf.variable_scope('trans', reuse=reuse):
                    ...
                    mask2 = 0.01*slim.conv2d(upcnv2, 3*num_source, [5, 5], stride=1, scope='mask2',
                        normalizer_fn=None, activation_fn=None)

                    upcnv1 = slim.conv2d_transpose(upcnv2, 16,  [7, 7], stride=2, scope='upcnv1')
                    mask1 = 0.01*slim.conv2d(upcnv1, 3*num_source, [7, 7], stride=1, scope='mask1',
                        normalizer_fn=None, activation_fn=None)
            else:
                mask1 = None
                mask2 = None
...
def pose_trans_u_net(tgt_image, src_image_stack, do_trans=True, is_training=True, reuse=False):
    ...
    with tf.variable_scope('pose_exp_net', reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                            ...
                            outputs_collections=end_points_collection):
            # cnv1 to cnv5b are shared between pose and explainability prediction
            #cnv1  = resnet_utils.conv2d_same(inputs, 16, 7, stride=2, scope='cnv1')
            #cnv2  = resnet_utils.conv2d_same(cnv1, 32,  5, stride=2, scope='cnv2')
            #cnv3  = resnet_utils.conv2d_same(cnv2, 64,  3, stride=2, scope='cnv3')
            #cnv4  = resnet_utils.conv2d_same(cnv3, 128, 3, stride=2, scope='cnv4')
            #cnv5  = resnet_utils.conv2d_same(cnv4, 256, 3, stride=2, scope='cnv5')
            cnv1  = slim.conv2d(inputs,16,  [7, 7], stride=2, scope='cnv1')
            cnv2  = slim.conv2d(cnv1, 32,  [5, 5], stride=2, scope='cnv2')
            cnv3  = slim.conv2d(cnv2, 64,  [3, 3], stride=2, scope='cnv3')
            cnv4  = slim.conv2d(cnv3, 128, [3, 3], stride=2, scope='cnv4')
            ...
            # Pose specific layers
            with tf.variable_scope('pose', reuse=reuse):
                cnv6  = resnet_utils.conv2d_same(cnv5, 256, 3, stride=2, scope='cnv6')
                cnv7  = resnet_utils.conv2d_same(cnv6, 256, 3, stride=2, scope='cnv7')
                pose_pred = slim.conv2d(cnv7, 6*num_source, [1, 1], scope='pred',
                    stride=1, normalizer_fn=None, activation_fn=None)
                pose_avg = 0.01*tf.reduce_mean(pose_pred, [1, 2])
                #pose_avg = tf.tanh(pose_avg)
                # Empirically we found that scaling by a small constant
                # facilitates training.
            ...
            if do_trans:
                with tf.variable_scope('trans', reuse=reuse):
                    upcnv5 = tf.image.resize_bilinear(cnv5, [H//16, W//16])
                    ...
                    upcnv2 = tf.image.resize_bilinear(upcnv3, [H//2, W//2])
                    upcnv2 = slim.conv2d(upcnv2, 32,  [5, 5], scope='upcnv2')
                    mask2 = 0.01*slim.conv2d(upcnv2, 3*num_source, [5, 5], stride=1, scope='mask2',
                        normalizer_fn=None, activation_fn=None)

                    upcnv1 = tf.image.resize_bilinear(upcnv2, [H, W])
                    upcnv1 = slim.conv2d(upcnv1, 16,  [7, 7], scope='upcnv1')
                    mask1 = 0.01*slim.conv2d(upcnv1, 3*num_source, [7, 7], stride=1, scope='mask1',
                        normalizer_fn=None, activation_fn=None)
            else:
                mask1 = None
...
def disp_bottleneck_d(end_points, concat, scope, skip_block, in_channels, out_channels, reuse=False, rates=[2,4], use_skip=True, use_skip2=True, skip2=None):
    with tf.variable_scope(scope, reuse=reuse):
        iH, iW = concat.get_shape()[1], concat.get_shape()[2]
        upnet = tf.image.resize_bilinear(concat, [iH*2, iW*2])
        #upnet = slim.conv2d_transpose(concat, in_channels, [3, 3], stride=2, scope='upnet', activation_fn=None)
        ...
        if use_skip:
            ...
            skip = tf.image.resize_bilinear(skip, [iH*2, iW*2])
        else:
        ...
        if use_skip2:
            ...
            #skip2 = slim.conv2d_transpose(skip2, out_channels, [3, 3], stride=2, scope='skip2', activation_fn=None)
...
def disp_bottleneck_dense(end_points, concat, scope, skip_block, in_channels, out_channels, reuse=False, rates=[2,4], use_skip=True, use_skip2=True, skip2=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        supnet = slim.conv2d(upnet, out_channels, [3, 3], rate=rates[0])
        aspp_up = slim.conv2d(upnet, out_channels, [3, 3], rate=rates[1])
        aspp_up2 = slim.conv2d(tf.concat([supnet, aspp_up], axis=3), out_channels, [3, 3], rate=rates[1])
        if use_skip:
            skip = slim.batch_norm(end_points[skip_block], activation_fn=tf.nn.elu)
            skip = slim.conv2d(skip, out_channels, [1, 1])
    ...
    with slim.arg_scope(resnet_utils.resnet_arg_scope(args.l2_regularizer, is_training,
        ...
        with tf.variable_scope("DeepLab_v3", reuse=reuse, custom_getter=custom_getter):
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose]):
                ...
                concat, _ = disp_bottleneck_d(end_points, upnet, "concat", args.resnet_model+"/block3", 1024, 512, reuse=reuse, use_skip2=False)
                concat1, skip1 = disp_bottleneck_d(end_points, concat, "concat1", args.resnet_model+"/block2", 512, 256, reuse=reuse, use_skip2=False)
                xconcat2, skip2 = disp_bottleneck_d(end_points, concat1, "concat2", args.resnet_model+"/block1", 256, 128, reuse=reuse, skip2=skip1)
                disp2 = DISP_SCALING*slim.conv2d(xconcat2, 1, [3, 3], activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp2') + MIN_DISP

                #upsampe3
                xconcat3, skip3 = disp_bottleneck_d(end_points, xconcat2, "concat3", args.resnet_model+"/block1", 128, 64, reuse=reuse, rates=[3,6], skip2=skip2)
                #for output
                disp3 = DISP_SCALING*slim.conv2d(xconcat3, 1, [3, 3], stride=1, activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp3') + MIN_DISP

                #upsample4
                xconcat4, _ = disp_bottleneck_d(end_points, xconcat3, "concat4", args.resnet_model+"/root_block", 64, 32, reuse=reuse, rates=[3,6], use_skip=False, skip2=skip3)
    ...
    with slim.arg_scope(resnet_utils.resnet_arg_scope(args.l2_regularizer, is_training,
        ...
        with tf.variable_scope("DeepLab_v3", reuse=reuse, custom_getter=custom_getter):
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose]):
                ...
                upnet = slim.conv2d(upnet, 1024, [1, 1], activation_fn=None)
                #upnet1 = slim.batch_norm(net, activation_fn=tf.nn.elu)
                concat, skip0 = disp_bottleneck_dense(end_points, upnet, "concat", args.resnet_model+"/block3", 1024, 512, reuse=reuse, use_skip2=False)
                concat1, skip1 = disp_bottleneck_dense(end_points, concat, "concat1", args.resnet_model+"/block2", 512, 256, reuse=reuse, skip2=skip0)
                xconcat2, skip2 = disp_bottleneck_dense(end_points, concat1, "concat2", args.resnet_model+"/block1", 256, 128, reuse=reuse, skip2=skip1)
                disp2 = DISP_SCALING*slim.conv2d(xconcat2, 1, [3, 3], activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp2') + MIN_DISP

                #upsampe3
                xconcat3, skip3 = disp_bottleneck_dense(end_points, xconcat2, "concat3", args.resnet_model+"/block1", 128, 64, reuse=reuse, rates=[3,6], skip2=skip2)
                #for output
                disp3 = DISP_SCALING*slim.conv2d(xconcat3, 1, [3, 3], stride=1, activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp3') + MIN_DISP
...
def disp_aspp_u_uni(inputs, args, is_training, reuse, size, custom_getter=None):
    ...
    with slim.arg_scope(resnet_utils.resnet_arg_scope(args.l2_regularizer, is_training,
                                                      ...
                                                      args.batch_norm_epsilon,
                                                      activation_fn=tf.nn.elu, use_transpose=True)):
        #with tf.variable_scope("disp_net", reuse=reuse):
        #    with slim.arg_scope([slim.conv2d], activation_fn=None, normalizer_fn=None):
        #        root_conv = resnet_utils.conv2d_same(inputs, 64, 7, stride=2, scope='root_conv')
        args.resnet_model = "resnet_v2_50"
        ...
        #manually add root conv
        #end_points[args.resnet_model + '/root_block'] = root_conv
        with tf.variable_scope("disp_net", reuse=reuse, custom_getter=custom_getter):
            # get block 4 feature outputs
            net = end_points[args.resnet_model + '/block4']
            net_size = tf.shape(net)[1:3]
            # resize the preact features
...
def pose_exp_u_resnet(tgt_image, src_image_stack, args, do_trans=True, is_training=True, reuse=False):
    #inputs = tf.concat([tgt_image, src_image_stack], axis=3)
    H = src_image_stack.get_shape()[1].value
    W = src_image_stack.get_shape()[2].value
    num_source = int(src_image_stack.get_shape()[3].value//3)
    with slim.arg_scope(resnet_utils.resnet_arg_scope(args.l2_regularizer, is_training,
                                                      args.batch_norm_decay,
                                                      args.batch_norm_epsilon,
                                                      activation_fn=tf.nn.elu, use_transpose=True)):
        #with tf.variable_scope('pose_exp_net', reuse=reuse):
        #    with slim.arg_scope([slim.conv2d], activation_fn=None, normalizer_fn=None):
        #        root_conv = resnet_utils.conv2d_same(inputs, 64, 7, stride=2, scope='pose_root_conv')

        args.resnet_model = "resnet_v2_50"
        _, end_points0 = resnet_v2.resnet_v2_50_slim(tf.slice(src_image_stack, [0, 0, 0, 0], [-1, -1, -1, 3]),
                                is_training=is_training,
                                num_units=[2,2,2,2],
                                global_pool=False,
                                spatial_squeeze=False,
                                output_stride=32,
                                reuse=True,
                                outputs_collections_name="src_imag0_resnet")

        _, end_points1 = resnet_v2.resnet_v2_50_slim(tf.slice(src_image_stack, [0, 0, 0, 3], [-1, -1, -1, 3]),
                                is_training=is_training,
                                num_units=[2,2,2,2],
                                global_pool=False,
                                spatial_squeeze=False,
                                output_stride=32,
                                reuse=True,
                                outputs_collections_name="src_img1_resnet")
        ...
        with tf.variable_scope('pose_exp_net', reuse=reuse) as sc:
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                                ...
                                outputs_collections=end_points_collection):
                net0 = slim.batch_norm(end_points0[args.resnet_model+'/block4'], activation_fn=tf.nn.elu)
                net0 = slim.conv2d(net0, 256, [1, 1], stride=1, scope="squeeze0", activation_fn=None)
                net1 = slim.batch_norm(end_points1[args.resnet_model+'/block4'], activation_fn=tf.nn.elu)
                net1 = slim.conv2d(net1, 256, [1, 1], stride=1, scope="squeeze1", activation_fn=None)
                net = slim.batch_norm(tgt_image[args.resnet_model+'/block4'], activation_fn=tf.nn.elu)
                net = slim.conv2d(net, 256, [1, 1], stride=1, scope="squeeze", activation_fn=None)
                net = tf.concat([net, net0, net1], axis=3)

                # Pose specific layers
                with tf.variable_scope('pose', reuse=reuse):
                    cnv6  = resnet_utils.conv2d_same(net, 256, 3, stride=2, scope='cnv6')
                    cnv7  = resnet_utils.conv2d_same(cnv6, 256, 3, stride=2, scope='cnv7')
                    pose_pred = 0.001*slim.conv2d(cnv7, 6*num_source, [1, 1], scope='pred',
                        stride=1, normalizer_fn=None, activation_fn=None)
                    pose_pred = tf.tanh(pose_pred)
                    pose_avg = tf.reduce_mean(pose_pred, [1, 2])
                    pose_final =  tf.reshape(pose_avg, [-1, num_source, 6])
                    np_scales = np.reshape(np.array([1.,1.,1., np.pi, np.pi, np.pi]), [1,1,6])
                    pose_scales = tf.constant(np_scales, dtype=tf.float32)
                ...
                if do_trans:
                    with tf.variable_scope('trans', reuse=reuse):
                        ...
                        upcnv5 = slim.conv2d(upcnv5, 128, [3, 3], stride=1, scope='upcnv5')
                        #cnv4 = slim.batch_norm(end_points[args.resnet_model+"/block3"], activation_fn=tf.nn.elu)
                        #cnv4 = slim.conv2d(cnv4, 128, [1, 1], stride=1, scope='cnv4', activation_fn=None)
                        #upcnv4 = tf.image.resize_bilinear(tf.concat([upcnv5, cnv4], axis=3), [H//8, W//8])
                        upcnv4 = tf.image.resize_bilinear(upcnv5, [H//8, W//8])
                        upcnv4 = slim.conv2d(upcnv4, 64, [3, 3], stride=1, scope='upcnv4')

                        #cnv3 = slim.batch_norm(end_points[args.resnet_model+"/block2"], activation_fn=tf.nn.elu)
                        #cnv3 = slim.conv2d(cnv3, 64, [1, 1], stride=1, scope='cnv3', activation_fn=None)
                        upcnv3 = tf.image.resize_bilinear(upcnv4, [H//4, W//4])
                        upcnv3 = slim.conv2d(upcnv3, 32, [3, 3], stride=1, scope='upcnv3')
                        mask3 = 0.01*slim.conv2d(upcnv3, 3*num_source, [3, 3], stride=1, scope='mask3',
                            normalizer_fn=None, activation_fn=None)
                        mask3 = tf.tanh(mask3)

                        #cnv2 = slim.batch_norm(end_points[args.resnet_model+"/block1"], activation_fn=tf.nn.elu)
                        #cnv2 = slim.conv2d(cnv2, 32, [1, 1], stride=1, scope='cnv2', activation_fn=None)
                        upcnv2 = tf.image.resize_bilinear(upcnv3, [H//2, W//2])
                        upcnv2 = slim.conv2d(upcnv2, 16, [3, 3], stride=1, scope='upcnv2')

                        mask2 = 0.01*slim.conv2d(upcnv2, 3*num_source, [5, 5], stride=1, scope='mask2',
                            normalizer_fn=None, activation_fn=None)
                        mask2 = tf.tanh(mask2)
                        #conv0 = slim.batch_norm(root_conv, activation_fn=tf.nn.elu)
                        #conv0 = slim.conv2d(conv0, 16, [1, 1], stride=1, scope='conv0', activation_fn=None)
                        upcnv1 = tf.image.resize_bilinear(upcnv2, [H, W])
